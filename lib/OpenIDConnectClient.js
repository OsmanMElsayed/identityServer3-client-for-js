 ...
 * @public
 * @class Config
 * @property {authorizationEndpoint}          Your target authorization endpoint
 * @property {logoutEndpoint}       Your target logout endpoint
 ...
 * @property {scope}       The scope of the endpoint you want to acquire an access token for
 */
...
AuthenticationContext = function (config) {
    ...
    this.CONSTANTS = {
        ...
        STORAGE: {
            ACCESS_TOKEN_KEY: 'openIDConnect.access.token',
            ID_TOKEN_KEY: 'openIDConnect.idtoken',
            ACCESS_TOKEN_EXPIRATION_KEY: 'openIDConnect.access.token.expiration.key',
            ID_TOKEN_EXPIRATION_KEY: 'openIDConnect.id.token.expiration.key',
            START_PAGE: 'openIDConnect.start.page',
            FAILED_RENEW: 'openIDConnect.failed.renew',
            STATE_LOGIN: 'openIDConnect.state.login',
            STATE_RENEW: 'openIDConnect.state.renew',
            STATE_IDTOKEN: 'openIDConnect.state.idtoken',
            NONCE_IDTOKEN: 'openIDConnect.nonce.idtoken',
            SESSION_STATE: 'openIDConnect.session.state',
            USERNAME: 'openIDConnect.username',
            ERROR: 'openIDConnect.error',
            ERROR_DESCRIPTION: 'openIDConnect.error.description',
            LOGIN_REQUEST: 'openIDConnect.login.request',
            LOGIN_ERROR: 'openIDConnect.login.error'
        },
        ERR_MESSAGES: {
    ...
    if (!config.logoutEndpoint) {
        throw new Error('logoutEndpoint is required');
    }
...
AuthenticationContext.prototype.login = function () {
    ...
    var scope = 'openid ' + this.config.scope;
    var urlNavigate = this._getNavigateUrl('id_token token', scope) + '&nonce=' + encodeURIComponent(this._idTokenNonce);
...
/**
 * Gets token from local storage cache
 * @param {string}   tokenType either CONSTANTS.ACCESS_TOKEN or CONSTANTS.ID_TOKEN.
 * @returns {string} token if exists and not expired or null
 */
AuthenticationContext.prototype.getCachedToken = function (tokenType) {
    
    var tokenTypeKey, expirationKey;
    if (tokenType == this.CONSTANTS.ACCESS_TOKEN) {
        tokenTypeKey = this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY;
        expirationKey = this.CONSTANTS.STORAGE.ACCESS_TOKEN_EXPIRATION_KEY;
    }
    else if (tokenType == this.CONSTANTS.ID_TOKEN) {
        tokenTypeKey = this.CONSTANTS.STORAGE.ID_TOKEN_KEY;
        expirationKey = this.CONSTANTS.STORAGE.ID_TOKEN_EXPIRATION_KEY;
    }
    else {
        return null;
    }
    
    var token = this._getItem(tokenTypeKey);
    var expired = this._getItem(expirationKey);
    ...
    if (expired && (expired > this._now() + offset)) {
        ...
        this._saveItem(tokenTypeKey, '');
        this._saveItem(expirationKey, 0);
        return null;
    }
};
...
AuthenticationContext.prototype.getCachedUser = function () {
    ...
    var idtoken = this._getItem(this.CONSTANTS.STORAGE.ID_TOKEN_KEY);
    this._user = this._createUser(idtoken);
    return this._user;
};

AuthenticationContext.prototype.registerCallback = function (expectedState, callback) {
    this._activeRenewal = expectedState;
 ...
 */
AuthenticationContext.prototype._renewToken = function (callback) {
    // use iframe to try refresh token
    this._logstatus('renewToken is called');
    
    var frameHandle = this._addFrame('renewFrame');
    var expectedState = this._guid();
    this._idTokenNonce = this._guid();
    this.config.state = expectedState;
    ...
    var urlNavigate = this._getNavigateUrl('token', this.config.scope) + '&prompt=none&login_hint=' + encodeURIComponent(this._user.userName);
    
    urlNavigate += '&nonce=' + encodeURIComponent(this._idTokenNonce);
    this.callback = callback;
    this.registerCallback(expectedState, callback);
    this.idTokenNonce = null;
    this._logstatus('Navigate to:' + urlNavigate);
    this._log('Navigate to:' + urlNavigate);
    this._saveItem(this.CONSTANTS.STORAGE.LOGIN_REQUEST, '');
    frameHandle.src = 'about:blank';
    this._loadFrame(urlNavigate, 'renewFrame');
};
...
AuthenticationContext.prototype._renewIdToken = function (callback) {
    ...
    var frameHandle = this._addFrame('idTokenFrame');
    var expectedState = this._guid();
    this._idTokenNonce = this._guid();
    this._saveItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN, this._idTokenNonce);
    this.config.state = expectedState;
    ...
    this._logstatus('Renew token Expected state: ' + expectedState);
    var urlNavigate = this._getNavigateUrl('id_token', 'openid') + '&prompt=none&login_hint=' + encodeURIComponent(this._user.userName);
    ...
    this._saveItem(this.CONSTANTS.STORAGE.LOGIN_REQUEST, '');
    frameHandle.src = 'about:blank';
    this._loadFrame(urlNavigate, 'idTokenFrame');
};
...
AuthenticationContext.prototype._loadFrame = function (urlNavigate, frameName) {
    ...
    setTimeout(function () {
        var frameHandle = self._addFrame(frameCheck);
 ...
 * @param {string}   tokenType  either CONSTANTS.ACCESS_TOKEN or this.CONSTANTS.ID_TOKEN
 * @param {requestCallback} callback
 */
AuthenticationContext.prototype.acquireToken = function (tokenType, callback) {
    
    if (!(tokenType == this.CONSTANTS.ACCESS_TOKEN || tokenType == this.CONSTANTS.ID_TOKEN)) {
        throw new Error('tokenType should be either CONSTANTS.ACCESS_TOKEN or this.CONSTANTS.ID_TOKEN');
    }
    
    var token = this.getCachedToken(tokenType);
    if (token) {
    ...
    //Already renewing, callback when we get the token.
    if (this._activeRenewal) {
        //Active renewals contains the state for each renewal.
        this.registerCallback(this._activeRenewal, callback);
    }
    else {
        var isIdTokenAcquisitionRequest = tokenType === this.CONSTANTS.ID_TOKEN;
        if (isIdTokenAcquisitionRequest) {
...
AuthenticationContext.prototype.clearCache = function () {
    ...
    this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_EXPIRATION_KEY, 0);
    this._saveItem(this.CONSTANTS.STORAGE.ID_TOKEN_KEY, '');
    this._saveItem(this.CONSTANTS.STORAGE.ID_TOKEN_EXPIRATION_KEY, 0);
    this._saveItem(this.CONSTANTS.STORAGE.FAILED_RENEW, '');
...
AuthenticationContext.prototype.logOut = function () {
    ...
    this.acquireToken(this.CONSTANTS.ID_TOKEN, function (error, idToken) {
        
        if (error) {
            throw new Error('failed to acquire the ID Token');
        }
        
        this.clearCache();
        this._user = null;
        
        var logoutParameters = '?id_token_hint=' + idToken;
        logoutParameters += '&post_logout_redirect_uri=' + encodeURIComponent(this.config.postLogoutRedirectUri);
        
        var urlNavigate = this.config.logoutEndpoint + logoutParameters;
...
AuthenticationContext.prototype.getRequestInfo = function (hash) {
    ...
    if (parameters) {
        ...
        if (parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION) ||
            ...
            // external api requests may have many simultaneous renewtoken requests       
            if (!requestInfo.stateMatch && this._renewStates.length > 0) {
                for (var i = 0; i < this._renewStates.length; i++) {
                    if (this._renewStates[i] === requestInfo.stateResponse) {
...
AuthenticationContext.prototype.saveTokenFromHash = function (requestInfo) {
    ...
    if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ERROR_DESCRIPTION)) {
        ...
        if (requestInfo.stateMatch) {
            ...
            this._logstatus('State matched successfully');
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ACCESS_TOKEN)) {
                ...
                // save token
                this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_KEY, requestInfo.parameters[this.CONSTANTS.ACCESS_TOKEN]);
                this._saveItem(this.CONSTANTS.STORAGE.ACCESS_TOKEN_EXPIRATION_KEY, this._expiresIn(requestInfo.parameters[this.CONSTANTS.EXPIRES_IN]));
            }
            ...
            if (requestInfo.parameters.hasOwnProperty(this.CONSTANTS.ID_TOKEN)) {
                ...
                if (this._user && this._user.profile) {
                    if (this._user.profile.nonce !== this._getItem(this.CONSTANTS.STORAGE.NONCE_IDTOKEN)) {
                        ...
                        this._saveItem(this.CONSTANTS.STORAGE.ID_TOKEN_KEY, requestInfo.parameters[this.CONSTANTS.ID_TOKEN]);
                        this._saveItem(this.CONSTANTS.STORAGE.ID_TOKEN_EXPIRATION_KEY, this._user.profile.exp);
                    }
                }
            }
        } else {
...
AuthenticationContext.prototype.handleWindowCallback = function () {
    ...
    if (this.isCallback(hash)) {
        ...
        this._log('returned from redirect url');
...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, scope) {
    
    var urlNavigate = this.config.authorizationEndpoint + this._serialize(responseType, this.config, scope);
...
AuthenticationContext.prototype._serialize = function (responseType, obj, scope) {
    var str = [];
    if (obj !== null) {
...
AuthenticationContext.prototype._addFrame = function (iframeId) {
    if (typeof iframeId === 'undefined') {
        return;
    }
    
    this._logstatus('Add frame to document:' + iframeId);
    ...
    if (!frame) {
        if (document.createElement && document.documentElement &&
            ...
            frame = document.getElementsByTagName('body')[0].appendChild(ifr);
        }
    ...
    frame.onpageshow = function (event) {
        var url = event.target.src;
        var isRedirectUrl = url.indexOf(this.config.redirectUri) >= 0;
        
        if (isRedirectUrl) {
            var urlParts = url.split('#');
            var urlHasHash = urlParts.length === 2;
            if (urlHasHash) {
                var hashFragment = urlParts[1];
                var currentLocation = window.location.href;
                var redirectUrl = currentLocation + '#' + hashFragment;
...
AuthenticationContext.prototype._log = function (message) {
